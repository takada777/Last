...
<div class="select-area">
 ...
 <br> <br> <br> <br> <br>
									...
									<label class="uk-form-label" for="form-stacked-text" >科目登録</label>
  <br> <br>

   <input type="text" placeholder="科目名" name="subjectname" id="subject_name" class=" uk-input uk-form-width-medium" id="form-stacked-text""/>
    <br>

    <br>
 <input type="button" class="uk-button uk-button-primary" value="登録" onclick="addsubject()">
</div>
</div>
</div>
...
socket.on("sql4", function(rows){addSql4(rows);});
...
function addSql4(rows){
	$("#sql4").html("");

	var table = '<span id="span2"></span><table class="uk-table uk-table-striped uk-table-large uk-width-xxlarge" >'
	+'<tr><th>科目名</th><th><div><input type="button" class="uk-button uk-button-default uk-button-small" id="button" value="削除" onclick="deletesubject()" ></div></th></tr>';
	//console.log(table);
	for( i = 0;i < rows.length;i++) {
		 test=rows[i].subject_id;
		table += '<tr>'+'<td>'+rows[i].subject_name+'</td><td>'+
				'<input type="checkbox" id=subject_id2 class="uk-checkbox" name="riyu" value='+test+'></td>';
		// 	$("#sql3").append('科目ID ' + rows[i].subject_id );
		//var test=document.rows[i].subject_id;
		 //	$("#sql3").append('科目名s ' + rows[i].subject_name + '<br>');
//console.log(test);
...
if(subject_name!=""){
		 socket.emit("addsubject",subject_name);
		 location.reload();
}else{
