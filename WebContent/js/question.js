var socket = io.connect("http://10.16.152.10:8081/question",{
	'reconnection': true,
...
function question_select(rows) {
	hairetu = rows;
	$("#qukekka").html("");
	$("#qu_name").append('<p class="uk-text-large">'+ rows[0].question_name +'</p>');
	var c;
	...
	for (var i = 0; i < rows.length; i++) {
		...
		list.push(data);
		c = parseInt(i + 1);
		$("#qukekka").append('<div class="uk-card uk-card-default uk-card-body uk-width-1-1"><h3 class="uk-card-title">問.'+ c +'</h3>'
								+ rows[i].question_text
								+'<div class="uk-margin"><input class="uk-input" type="text" placeholder="解答"></div></div><br>');

	}
	$("#qukekka").append('<div class="uk-flex uk-flex-center@m uk-flex-right@l"><a class="uk-button uk-button-default uk-margin-left" href="#" onclick="saiten()">採点</a></div>');
};
