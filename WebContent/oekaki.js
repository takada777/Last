...
var socket = io.connect("http://10.16.152.10:8081/oekaki",{
	'reconnection': true,
...
if (('sessionStorage' in window)
	...
	if(get_storage = sessionStorage.getItem('user_name') == null){

	} else {
		user_name = get_storage = sessionStorage.getItem('user_name');
	}
} else {
...
socket.on("msg_log", function(json) {
	console.log(json);
	msg_log(json);
});
...
function msg_log(json) {
	...
	if(json.data.length == 0) {
		...
		for(var i = 0;i < json.data.length;i++) {
			console.log("msg_2")
			if(json.data[i].name == user_name) {
				$("#msg").append('<div class="line__right"><div class="text">' +  json.data[i].msg + ' <span class="date">'+ json.data[i].time +'</span></div></div></div>');
			} else {
				$("#msg").append('<div class="line__left"><div class="line__left-text"><div class="name">'+ json.data[i].name +'</div><div class="text">'+ json.data[i].msg +'<span class="date">'+ json.data[i].time +'</span></div></div></div>');
			}
...
document.getElementById("send").onclick = function() {
	...
	var Minutes = DD.getMinutes();
	var time = Hours +":"+Minutes;
	$("#msg").append('<div class="line__right"><div class="text">' + msg + ' <span class="date">'+ time +'</span></div></div></div>');
	$(".line__contents.scroll").scrollTop( $(".line__contents.scroll")[0].scrollHeight );
	socket.emit("line", msg,roomid,user_name,time);
...
canvas.on('path:created', function(e) { // or 'object:added'
	...
	socket.emit("object", i[i.length - 1], data, roomid, user_name);
